...
def update_history(name: str, command: str, **kwargs):
    ...
    if command == 'create':
        ...
        for k in ('test', 'export', 'export pvalues', 'export raw_pvalues'):
            if k in d:
                del d[k]
...
def reproduce(filename: str):
    with open(filename, 'r') as f:
        d = json.load(f)
    name = d['name']
    if d['betanegbinfit'] != bnb_version:
        print(f'[yellow]Warning:[/yellow] Current [bold]betanegbinfit[/bold] version is {bnb_version}, but the project was created with'
              f' {d["betanegbinfit"]}.')
    if d['jax'] != jax_version:
        print(f'[yellow]Warning:[/yellow] Current [bold]JAX[/bold] version is {jax_version}, but the project was created with'
              f' {d["jax"]}.')
    if d['mixalime'] != mixalime_version:
        print(f'[yellow]Warning:[/yellow] Current [bold]MixALime[/bold] version is {mixalime_version}, but the project was created with'
              f' {d["mixalime"]}.')
    if d['dill'] != dill_version:
        print(f'[yellow]Warning:[/yellow] Current [bold]dill[/bold] version is {dill_version}, but the project was created with'
              f' {d["dill"]}.')
...
def _create(name: str = Argument(..., help='Project name. [bold]MixALime[/bold] will produce files for internal usage that start with [cyan]'
    ...
    _, samples, snvs = create_project(name=name, snvs=files, bad_maps=bad_maps, default_bad=default_bad, drop_bads=drop_bad, min_qual=min_qual,
                                      ...
                                      filter_name=filter_name, compression=compression, count_snvs=True)
    ...
    for bad in sorted(samples):
        a = len(snvs[bad])
        b = samples[bad]
        table.add_row('{:.2f}'.format(bad), str(a), str(b))
        tot_snv += a
        tot_samples += b
    print(table)
    ...
    update_history(name, 'create', files=files, bad_maps=bad_maps, default_bad=default_bad, drop_bad=drop_bad, min_qual=min_qual,
                   min_cnt=min_cnt, cnt_max_sum=cnt_max_sum, filter_db=filter_db, filter_rs=filter_rs, symmetrify=symmetrify,
                   filter_name=filter_name, compression=compression)
    dt = time() - t0
    print(f'[green][bold]✔️[/bold] Done![/green]\t time: {dt:.2f} s.')
...
def _fit(name: str = Argument(..., help='Project name.'),
    ...
    t0 = time()
    with Progress(SpinnerColumn(speed=0.5), TextColumn("[progress.description]{task.description}"), transient=True) as p:
...
def _combine(name: str = Argument(..., help='Project name.'),
             ...
             id_startswith: str = Option(None, help='Only SNVs whose IDs start with this strings are tested (e.g. "rs").'),
    ...
    with Progress(SpinnerColumn(speed=0.5), TextColumn("[progress.description]{task.description}"), transient=True) as p:
        ...
        r = combine(name, group_files=group, alpha=alpha, id_startswith=id_startswith, subname=subname, n_jobs=n_jobs)[subname]
    ...
    print(table)
    update_history(name, 'combine', group=group, alpha=alpha, id_startswith=id_startswith, subname=subname, n_jobs=n_jobs)
    dt = time() - t0
    print(f'[green][bold]✔️[/bold] Done![/green]\t time: {dt:.2f} s.')
...
def _counts(name: str = Argument(..., help='Project name.'), out: str = Argument(..., help='Output filename/path.'),
    ...
    print('[green][bold]✔️[/bold] Done![/green]')
